  ...
  return (
    <div className='h-full overflow-y-auto p-6 flex items-start flex-wrap gap-4 text-gray-300'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-gray-800 rounded-lg border border-gray-700'>
       ...
       <input onChange={(e)=>setInput(e.target.value)} value={input} type="text" className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-600 bg-gray-900 text-white placeholder-gray-500'  placeholder='The future of artificial intelligence is...' required/>

       <p className='mt-4 text-sm font-medium text-gray-300'>Article Length</p>

       <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
        {articleLenght.map((item,index)=>(
          <span onClick={() =>setSelectedLength(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text===item.text ? 'bg-blue-500/10 border-blue-500 text-blue-400' : 'text-gray-400 border-gray-600 hover:bg-gray-700'}`} key={index}>{item.text}</span>
        ))}
       </div>
       ...
       <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#226BFF] to-[#65ADFF] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer disabled:opacity-50'>
{
  ...
  :<Edit className='w-5'/>
      ...
      {/*right-col*/}
      <div className='w-full max-w-lg p-4 bg-gray-800 rounded-lg flex flex-col border border-gray-700 min-h-96 max-h-[600px]'>
<div className='flex items-center gap-3'>
...
{!content  ? ( 
  ...
  <div className='mt-3 h-full overflow-y-scroll text-sm text-gray-300 prose prose-invert prose-sm max-w-none'>

      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-gray-800 rounded-lg border border-gray-700'>
       ...
       <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
        {blogCategories.map((item)=>(
          <span onClick={() =>setSelectedCategory(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedCategory===item ? 'bg-purple-500/10 border-purple-500 text-purple-400' : 'text-gray-400 border-gray-600 hover:bg-gray-700'}`} key={item}>{item}</span>
        ))}
       </div>
